...
#define paramaters
tf.app.flags.DEFINE_string('load_path','../data/AllSampleSet/non_cancer_subset00/',"directory which include original cancer or no-cancer pictures")
...
tf.app.flags.DEFINE_float('IOU_threshold', 0.8, "step in vertical direction")
...
def IOUType(FileName, x_top, y_top, x_bottom, y_bottom):
    ...
    if(os.path.exists(FLAGS.cc_path + ccFileName)==False):
	return 0
...
def Split2Blocks(FileName):
    ...
    for i in range(0, yRange):
        ...
        x_top = 0 + FLAGS.offsetX
        x_bottom = FLAGS.block_width + FLAGS.offsetX
        y_top = y_top + FLAGS.stepY + FLAGS.offsetY
        y_bottom = y_bottom + FLAGS.stepY + FLAGS.offsetY
